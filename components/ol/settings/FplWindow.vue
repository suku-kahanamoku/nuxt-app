...
<template>
	<v-btn v-if="open" color="primary" class="my-1" @click="open = !open">{{ $t('settings.fpl_window') }}</v-btn>
	<v-btn v-else outline="primary" class="my-1" @click="open = !open">{{ $t('settings.fpl_window') }}</v-btn>

	<Teleport v-if="open" :to="`#${wrapId}`">
		<DraggableResizable :config="config" @change="onChange" @close="open = false">
			<template v-slot:header>
				{{ $t('settings.fpl_window') }}
			</template>
