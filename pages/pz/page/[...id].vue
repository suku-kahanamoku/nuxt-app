<script setup lang="ts">
	...
	const pageConfig = CLONE((useState('pages').value as any).page_detail);
	...
	async function load() {
		if (configs?.cmp_page_form?.submitUrl) {
			const result = await useApi(configs.cmp_page_form.submitUrl);
			data.value = result[0];
		} else {
			data.value = {};
		}
	...
	<div>
		...
		<v-window v-model="tab">
			<v-window-item value="field">
				<Form v-if="configs?.cmp_page_form" :config="configs?.cmp_page_form" :data="data" @submit="onSubmit" />
			</v-window-item>

			<v-window-item value="json">
				<JsonForm
					v-if="configs?.cmp_page_form"
					:config="configs?.cmp_page_form"
					:data="data"
					@submit="onSubmit"
				/>
			</v-window-item>
