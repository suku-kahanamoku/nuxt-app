<script setup lang="ts">
	...
	definePageMeta({
		syscode: 'page_profile_detail',
	});

	const pageConfig = CLONE((useState('pages').value as any).page_profile_detail);
	...
	async function load() {
		if (configs?.cmp_profile_form?.submitUrl) {
			const result = await useApi(configs.cmp_profile_form.submitUrl);
			data.value = result[0];
		} else {
			data.value = {};
		}
	...
	<div>
		...
		<v-window v-model="tab">
			<v-window-item value="field">
				<Form
					v-if="configs?.cmp_profile_form"
					:config="configs?.cmp_profile_form"
					:data="data"
					@submit="onSubmit"
				/>
			</v-window-item>
